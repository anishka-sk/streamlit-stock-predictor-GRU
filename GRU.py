...
st.title("GRU Stock Price Predictor 📊")
st.markdown("This application predicts stock prices (Open, High, Low, Close, Volume) using a standalone GRU deep learning model.")
...
if st.sidebar.button("Run Prediction"):
    ...
    if df is not None and not df.empty:
        ...
        st.subheader("Building and Training the GRU Model")
        with st.spinner("Training GRU model..."):
            gru_model = Sequential()
            gru_model.add(GRU(128, return_sequences=True, input_shape=(look_back, len(features))))
            gru_model.add(Dropout(0.3))
            gru_model.add(GRU(128, return_sequences=False))
            gru_model.add(Dropout(0.3))
            gru_model.add(Dense(64))
            gru_model.add(Dense(len(features)))  # predict 5 outputs
            gru_model.compile(optimizer='adam', loss='mean_squared_error')

            early_stopping = EarlyStopping(monitor='val_loss', patience=15, restore_best_weights=True)
            history = gru_model.fit(X_train, y_train, batch_size=batch_size, epochs=epochs,
        ...
        fig_val.update_layout(
            title="Model Validation: Actual vs. Predicted Close Prices",
            xaxis_title="Date",
            yaxis_title="Price",
        ...
        for _ in range(future_days):
            ...
            pred = gru_model.predict(X_future, verbose=0)[0]
